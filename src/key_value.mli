...
(** Default implementation of {!Sigs.VALUE}, used in {!MakeMap}.
    @since 0.10.0 *)

module HomogeneousValue : HETEROGENEOUS_VALUE with type ('a,'map) t = 'map
(** Default implementation of {!Sigs.HETEROGENEOUS_VALUE}, to use when the type of the
...
(** Generic implementation of {!Sigs.HASHED_VALUE}.
    ...
    ({!MakeHashconsedMap.to_int}), see the documentation of {!Sigs.HASHED_VALUE.polyeq}
    for details on this. *)

module HeterogeneousHashedValue : HETEROGENEOUS_HASHED_VALUE with type ('k, 'm) t = 'm
(** Generic implementation of {!Sigs.HETEROGENEOUS_HASHED_VALUE}.
    Uses {{: https://ocaml.org/api/Hashtbl.html#VALhash}[Hashtbl.hash]} for hashing
    and physical equality for equality.
    Note that this may lead to maps of different types having the same identifier
    ({!MakeHashconsedHeterogeneousMap.to_int}), see the documentation of
    {!Sigs.HASHED_VALUE.polyeq} for details on this. *)
...
(** Create a {!Sigs.HETEROGENEOUS_KEY} from a non-polymorphic {!Sigs.KEY} *)
...
(** Create a {!Sigs.HETEROGENEOUS_HASHED_VALUE} from a {!Sigs.HASHED_VALUE} *)
